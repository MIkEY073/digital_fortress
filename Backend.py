...
class PasswordCheckResponse(BaseModel):
    score: int              # 0–100
    rating: str             # "Very Weak" / "Weak" / "OK" / "Strong" / "Very Strong"
...
class SaveCredentialsRequest(BaseModel):
    user_id: int
    user_name: str
    passw: str          # plain text from frontend, we will hash it
    platform: str
    strength_score: int
...
class UpdatePlatformPasswordRequest(BaseModel):
    ...
    If platform credentials exist → UPDATE.
    If not → INSERT a new row (user_name is reused if row exists; otherwise
    platform name is used as a placeholder).
    """
    user_id: int
    platform: str
    new_password: str
...
class DeleteCredentialsRequest(BaseModel):
    """
    Delete one credential row for a user/platform/username.
    """
    user_id: int
    platform: str
    user_name: str
...
def save_credentials(req: SaveCredentialsRequest):
    """
    Save credentials for a given user + platform.
    Password is stored as a bcrypt hash (like user_info.passw).
    """
    conn = get_connection()
    cur = conn.cursor()
...
def update_platform_password(req: UpdatePlatformPasswordRequest):
    ...
    - If a row exists for (user_id, platform) → update passw + strength_score.
    - If not → insert a new row using platform as a placeholder user_name.
    """
    conn = get_connection()
    cur = conn.cursor()
    ...
    # Check existing credentials for this platform
    cur.execute(
    ...
    if row:
        ...
        # INSERT new row (user_name placeholder = platform name)
...
def delete_credentials(req: DeleteCredentialsRequest):
    """
    Delete credentials from DB for this user/platform/username combination.
    Safe even if no row exists (it just deletes 0 rows).
    """
    conn = get_connection()
    cur = conn.cursor()
...
# ---------- WEBSITE SAFETY CHECK (PHISHTANK + RULES) ----------
